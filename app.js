...
window.addEventListener('load', () => {
  ...
  const savedYear = localStorage.getItem('selectedYear');
    ...
    if (savedMake && savedModel && savedYear) {
    setTimeout(() => {
...
yearSelect.addEventListener('change', () => {
 ...
 <div class="animate-fade-in">
    ...
    <ul class="list-disc pl-6 space-y-2">
      ...
      ${specs.sparkPlugs ? `<li>⚡ <strong>Spark Plugs:</strong> ${specs.sparkPlugs}</li>` : ""}
      ${specs.caliperBolts ? `<li>🛑 <strong>Brake Caliper Bolts:</strong> ${specs.caliperBolts}</li>` : ""}
      ${specs.transPanBolts ? `<li>⚙️ <strong>Transmission Pan Bolts:</strong> ${specs.transPanBolts}</li>` : ""}
    </ul>
  </div>
    `;
  } else {
...
applyBtn.addEventListener('click', () => {
  try {
    ...
    torqueSpecs = updated;
    feedback.textContent = 'Updated in-memory torque data. Use dropdowns to preview.';
